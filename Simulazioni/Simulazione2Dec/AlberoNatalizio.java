...
public class AlberoNatalizio implements Iterable<Decorazione> {
    ...
    public void accendi() {
        ...
        ArrayList<DecorazioneElettrica> decorazioniElettriche = new ArrayList<>(decorazioni.stream()
                .filter(t -> t instanceof DecorazioneElettrica).map(t -> (DecorazioneElettrica) t).toList());
    ...
    public double getSommaPotenze() {
        ...
        return decorazioni.stream()
                .filter(t -> ((t instanceof DecorazioneElettrica) && ((DecorazioneElettrica) t).isAccesa()))
                .map(t -> (DecorazioneElettrica) t).mapToDouble(DecorazioneElettrica::getPotenza)
                .reduce(0, (s, el) -> s + el);
